...
import { useLoad, login, getUserProfile, getStorageSync, setStorageSync, removeStorageSync, request, showToast, getSystemInfo, getNetworkType, chooseImage, hideLoading, showActionSheet } from '@tarojs/taro'
...
const fetchUserInfo = async () => {
  try {
    console.log('调用/api/user/info接口获取用户信息...')
    ...
    const response = await request({
      url: 'http://localhost:8082/api/user/info',
...
const checkLoginStatus = async () => {
    try {
      ...
      if (token) {
        ...
        if (userInfo) {
          ...
          // 获取用户信息失败，可能是token失效，清除状态
          setUserInfo(null)
          setIsLogin(false)
          removeStorageSync('token')
          // 不再单独存储userId，所以不需要移除
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const response = await request({
        url: 'http://localhost:8082/api/user/login', // 本地开发环境后端地址
        method: 'POST',
        data: requestData,
        header: {
          'content-type': 'application/json'
      ...
      const fullBackendAvatarUrl = backendAvatarUrl && backendAvatarUrl.startsWith('/api/') ? 
        `${baseUrl}${backendAvatarUrl}` : backendAvatarUrl;
  ...
  const uploadAvatar = async () => {
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        wx.uploadFile({
          url: 'http://localhost:8082/api/user/avatar/upload',
          ...
          success: (res) => {
            ...
            if (res.statusCode === 200 && data.success) {
              ...
              const baseUrl = 'http://localhost:8082';
              const newAvatarUrl = data.data.accessUrl.startsWith('/api/') ? 
                `${baseUrl}${data.data.accessUrl}` : data.data.accessUrl;
        ...
        <View className='user-details'>
          {isLogin ? (
            <>
              ...
              <Text className='user-id'>ID: {userInfo?.userId || '未知'}</Text>
              <Button className='edit-btn' size='mini' onClick={() => showToast({title: '功能开发中'})}>编辑资料</Button>
            </>
          ) : (
      ...
      {isLogin && (
        <View className='goal-section'>
          <View className='goal-header'>
            <Text className='goal-title'>我的学习目标</Text>
            <Button className='modify-btn' size='mini' onClick={() => showToast({title: '功能开发中'})}>修改</Button>
          </View>
          <View className='goal-content'>
            <Text className='goal-name'>雅思</Text>
            <Text className='goal-category'>外语 {'>'} 英语</Text>
            <View className='progress-bar'>
              <View className='progress' style={{ width: '35%' }}></View>
            </View>
            <Text className='progress-text'>35% 完成</Text>
          </View>
        </View>
      )}
          ...
          {isLogin ? (
            <View className='function-list'>
              ...
              <View className='function-item' onClick={() => showToast({title: '功能开发中'})}>
                <View className='function-icon'>📊</View>
                <View className='function-info'>
                  <Text className='function-name'>学习统计</Text>
                  <Text className='function-desc'>详细数据分析</Text>
                </View>
                <View className='arrow-right'>→</View>
              </View>
              <View className='function-item' onClick={() => showToast({title: '功能开发中'})}>
                <View className='function-icon'>🏆</View>
                <View className='function-info'>
                  <Text className='function-name'>我的成就</Text>
                  <Text className='function-desc'>查看获得徽章</Text>
                </View>
                <View className='arrow-right'>→</View>
              </View>
              <View className='function-item' onClick={() => showToast({title: '功能开发中'})}>
              ...
              <View className='function-item' onClick={() => showToast({title: '功能开发中'})}>
                <View className='function-icon'>📝</View>
                <View className='function-info'>
                  <Text className='function-name'>学习笔记</Text>
                  <Text className='function-desc'>记录学习心得</Text>
                </View>
                <View className='arrow-right'>→</View>
              </View>
              <View className='function-item' onClick={() => showToast({title: '功能开发中'})}>
                <View className='function-icon'>📅</View>
                <View className='function-info'>
                  <Text className='function-name'>学习计划</Text>
                  <Text className='function-desc'>制定学习目标</Text>
                </View>
                <View className='arrow-right'>→</View>
              </View>
              <View className='function-item' onClick={() => showToast({title: '功能开发中'})}>
